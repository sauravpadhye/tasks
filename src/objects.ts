...
export function isValid(question: Question, answer: string): boolean {
    if (question.type === "short_answer_question") {
        ...
        const test = question.options.includes(answer);
        return test;
    }
    return true;
}
...
export function toShortForm(question: Question): string {
    const long = question.id.toString() + ": " + question.name;
    const short = long.slice(0, 14);
    return short;
}
...
export function publishQuestion(question: Question): Question {
    const pubQ = { ...question, published: !question.published };
    return pubQ;
}
...
export function addOption(question: Question, newOption: string): Question {
    ...
    const addedQ = { ...question, options: newOptions };
    return addedQ;
}
